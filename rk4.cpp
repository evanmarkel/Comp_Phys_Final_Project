...
void RK4::integrate(std::valarray<double> &X, std::valarray<double> &V, double dt, SolarSystem mysystem, double G, double eps)
{
    ...
    k1 = mysystem.calculateForcesAndEnergy(X, V, G, eps) * dt;
    k2 = mysystem.calculateForcesAndEnergy(X + 0.5 * k1, V, G, eps) * dt;
    k3 = mysystem.calculateForcesAndEnergy(X + 0.5 * k2, V, G, eps) * dt;
    k4 = mysystem.calculateForcesAndEnergy(X + k3, V, G, eps) * dt;
